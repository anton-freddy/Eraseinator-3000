...
/****************************************************
  Method: AS5600_ENC
  In: Wire instance (I2C Bus)
  Out: none
  Description: constructor class for AMS 5600
*****************************************************/
AS5600_ENC::AS5600_ENC(TwoWire &I2C)
{
  ptr = &I2C;
}
...
/****************************************************
  ...
  In: Wire instance (I2C Bus)
  Out: none
  Description: Set I2C bus (Wire) to use
*****************************************************/
void AS5600_ENC::setWire(TwoWire &I2C)
{
  *this->ptr = I2C;
}
...
{
  int retVal = -1;
  ptr->beginTransmission(_ams5600_Address);
  ptr->write(in_adr);
  ptr->endTransmission();
  ptr->requestFrom(_ams5600_Address, (uint8_t)1);
...
word AS5600_ENC::readTwoBytesTogether(int addr_in)
{
  ...
  ptr->write(addr_in);
  ptr->endTransmission();
  ptr->requestFrom(_ams5600_Address, (uint8_t)2);
  while (ptr->available() < 2)
    ;

  int highByte = ptr->read();
  int lowByte = ptr->read();
